...
import { RotateCcw, Users, Globe, LogOut, Shield, Wand2, Wallet, Receipt, Bot, PieChart } from 'lucide-react';
...
export const StickyHeader: React.FC<StickyHeaderProps> = ({ 
  ...
  const steps = [
    { id: AppStep.SALARY, label: t.step1, icon: Wallet },
    { id: AppStep.WIZARD, label: t.stepWizard, icon: Wand2 },
    { id: AppStep.ADVISOR, label: t.advisorStep, icon: Bot },
    { id: AppStep.EXPENSES, label: t.step2, icon: Receipt },
    { id: AppStep.DASHBOARD, label: t.step3, icon: PieChart },
  ];
                ...
                <div className="flex items-center justify-between md:justify-center bg-white rounded-[2rem] border border-slate-200/80 shadow-sm p-3 px-4 sm:px-8 w-max min-w-full md:min-w-0 mx-auto gap-1 sm:gap-4">
                    {steps.map((step, idx) => {
                        ...
                        <React.Fragment key={step.id}>
                            ...
                            {idx < steps.length - 1 && (
                                <div className={`h-[2px] w-8 sm:w-12 md:w-16 shrink-0 -mt-6 transition-colors duration-500 rounded-full ${isCompleted ? 'bg-blue-500' : 'bg-slate-200'}`}></div>
                            )}
                        </React.Fragment>
                    )})}
                </div>
